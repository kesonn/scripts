...
#配置区
#使用第几块网卡 根据实际情况调整  0代表第一块 1代表第二块 以此类推
RET = 0
...
class Utils(object):
    ...
    def outbox(self,*ss):
        ...
        self.lock.release()
    @classmethod
    def inbox(self,s,df=None):
        """
        #s --- 输入时显示的提示信息
        #df --- 默认值
        """
        self.lock.acquire()
        sys.stdout.write(s+u'(默认%s >'%str(df))
        sys.stdout.flush()
    ...
    @staticmethod
    def getlocalip():
        return socket.gethostbyname_ex(socket.gethostname())[2][RET]
...
def login(obj,AUTO_LOGIN=True):
    ...
    if not obj.info.get('wlanuserip'):
        obj.info['wlanuserip'] = localip if AUTO_LOGIN else Utils.inbox(u'请输入你的IP地址',df=localip)
    obj.getinit()
    obj.info['USER'] = user if AUTO_LOGIN else Utils.inbox(u"请输入手机号码",df=user)
    if not AUTO_LOGIN:
        obj.sendsms()
    obj.info['PWD'] = ' '*4 if AUTO_LOGIN else Utils.inbox(u"请输入收到的短信验证码",df=' '*4)
    while True:
        if AUTO_LOGIN:
            obj.info['USER'] = random.randint(13800000000,13800138000)
        obj.login()
...
def changeip(obj):
    while True:
        if msvcrt.kbhit():
            ...
            #print(key)
            if key == b'\x11':  #ctrl+q
                userip = Utils.inbox(u'请输入你的IP地址', df = Utils.getlocalip())
                if userip:
